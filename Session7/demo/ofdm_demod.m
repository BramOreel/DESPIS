...
%Determine a first channel equalization based on the training packet
...
%Initialise the adaptive filter variables, we will store the channel
%estimates
iterations = size(QAM_matrix_usefull,2) - Lt;
w = 1./conj(CHANNEL);
rec_Xk = zeros(1,N/2-1);
Ek = zeros(1,N/2-1);

for step = 1:iterations
    %fetch received symbols
    Yk = QAM_matrix_data(:,step);



    %Calculate X after aplying the filter correction
    estXk = conj(w).*Yk;
    %Append the data to the QAM_matrix
    QAM_matrix = [QAM_matrix , estXk];
    %reconstruct transmitted signal for each frequency
    rec_Xk = qammod(qamdemod(estXk,M),M);
    %Calculate the error
    Ek =  estXk - rec_Xk;
    %Update the filter
    w = w + mu./(alpha + conj(Yk).*Yk).*Yk.*conj(rec_Xk - conj(w).*Yk);
    %append the channel estimation to the matrix
    CHANNELS = [CHANNELS,1./conj(w)];
end
